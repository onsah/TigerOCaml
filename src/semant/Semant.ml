...
let expecting_ty expected_ty given_ty pos =
  ...
  | false ->
      TigerError.semant_error
        ( Printf.sprintf "Type mismatch: Expected type %s, found %s"
...
let rec transExpr (value_env, type_env, Syntax.Expr {expr; pos}) =
  ...
  | Syntax.CallExpr {func; args} -> (
      ...
      | Env.FunEntry {argTypes; return_type} ->
          ...
          in
          type_check_args argTypes
            (List.map (fun arg -> arg.ty) args_checked)
            pos ;
          {translated_expr= {translated_expr= (); pos}; ty= return_type}
      | other ->
          TigerError.semant_error
            ( "Variable " ^ Symbol.name func
  ...
  | Syntax.RecordExpr {typ; fields} -> (
      ...
      | Types.Record (expected_field_types, _) ->
          ...
          in
          let _ =
            type_check_fields field_names_with_types expected_field_types typ
              pos
          in
          {translated_expr= {translated_expr= (); pos}; ty= record_ty}
      | found_ty ->
          TigerError.semant_error
            ( "Type " ^ Symbol.name typ ^ " is " ^ Types.show_ty found_ty
        ...
        | false ->
            TigerError.semant_error
              ( Printf.sprintf
                  ...
                  (Types.show_ty left_ty) (Types.show_ty right_ty)
              , pos ) )
    | _ ->
...
and type_check_args arg_types args pos =
  let arg_types_len = List.length arg_types and args_len = List.length args in
  ...
  | true -> (
      ...
      | Some (arg_type, arg) ->
          TigerError.semant_error
            ( "Types " ^ Types.show_ty arg_type ^ " and " ^ Types.show_ty arg
              ^ " are different"
            , pos )
      | None ->
          () )
  ...
  List.map
    (fun (name, given_field_opt, expected_field_opt) ->
      ...
      | Some (_, given_ty, _), Some {field_ty= expected_ty; _} ->
          if given_ty = expected_ty then (name, given_ty)
          else
            TigerError.semant_error
              ( sprintf "Type mismatch, expected type %s, but found %s"
                  (Types.show_ty given_ty)
                  (Types.show_ty expected_ty)
              , pos )
      | Some (given_name, given_ty, pos), None ->
          TigerError.semant_error
            ( sprintf "Given gield %s does not exist on record on record %s"
                (Types.show_field {field_id= given_name; field_ty= given_ty})
                (Symbol.name record_name)
            , pos )
      | None, Some expected_ty ->
          TigerError.semant_error
            ( sprintf "Expected field %s in record on record %s"
      ...
      TigerError.semant_error
        ( Printf.sprintf
            "Type mismatch, assigned variable has type %s, but value is of \
             type %s"
            (Types.show_ty var_ty) (Types.show_ty expr_ty)
        , pos )
...
and handle_if_expr value_env type_env (cond, then_arm, else_arm_opt) if_pos =
  ...
  | Some else_arm ->
      ...
      if then_ty = else_ty then
        {translated_expr= {translated_expr= (); pos= if_pos}; ty= then_ty}
      else
        TigerError.semant_error
          ( Printf.sprintf
              "If arms does not match. Then arm is type of %s and else is %s"
              (Types.show_ty then_ty) (Types.show_ty else_ty)
          , pos )
  | None ->
...
and handle_for_expr value_env type_env (var, from, to', body) pos =
  ...
  in
  let _ = expecting_int from_ty from_pos and _ = expecting_int to_ty to_pos in
          ...
          | None ->
              TigerError.semant_error
                ( Printf.sprintf "Field %s does not exist on type %s"
                    (Symbol.name field_id) (Types.show_ty var_ty)
                , pos ) )
      | _ ->
          TigerError.semant_error
            ( Printf.sprintf
          ...
          | false ->
              TigerError.semant_error
                ( Printf.sprintf
                    ...
                    (Types.show_ty decl_ty) (Types.show_ty value_ty)
                , pos ) ) )
