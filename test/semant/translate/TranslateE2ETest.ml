...
let test_conitional_or_if _ =
  let expr =
    Result.get_ok
      (Main.analyze_file ~path:"../../scripts/translate/conditional_or_if.tig")
  in
  ignore (assert_equal expr.ty Types.Int) ;
  ignore
    ( match expr.translated_expr.translated_expr with
    | Expr
        (ESeq
          ( Seq
              [ IRTree.CondJump
                  { true_label = true_label_1'; false_label = false_label'; _ }
              ; IRTree.Label false_label_1
              ; IRTree.CondJump
                  { true_label = true_label_2'; false_label = false_label''; _ }
              ; IRTree.Label false_label_2
              ; IRTree.Move { location = IRTree.Temp result'; value = Const 0 }
              ; IRTree.Jump { labels = [ join_label' ]; _ }
              ; IRTree.Label true_label
              ; IRTree.Move { location = IRTree.Temp result''; value = Const 1 }
          ...
          , IRTree.Temp result ) )
      when false_label' = false_label_1
           && true_label_1' = true_label
           && false_label'' = false_label_2
           && true_label_2' = true_label
           && join_label' = join_label
           && result' = result
           && result'' = result ->
        assert_bool "matched pattern" true
...
let suite =
  ...
  >::: [ "if_else should process if expressions normally" >:: test_regular_if
       ; "if_else should optimize if expressions generated from and expressions"
         >:: test_conitional_and_if
       ; "if_else should optimize if expressions generated from or expressions"
         >:: test_conitional_or_if
       ]
